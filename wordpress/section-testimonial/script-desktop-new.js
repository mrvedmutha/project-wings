(function () {
  ...
  function init() {
    if (typeof gsap === "undefined") {
      console.warn("Wings testimonials (desktop): GSAP not found.");
      return;
    }

    const wrapper = document.querySelector(
      ".homepage-testimonial__cards-wrapper",
    );
    if (!wrapper) return;

    const pages = Array.from(
      wrapper.querySelectorAll(".homepage-testimonial__cards"),
    );
    ...
    /* insert the viewport before the first page */
    pages[0].parentNode.insertBefore(viewport, pages[0]);
    /* move every page inside it */
    ...
    function getCards(page) {
      return Array.from(
        page.querySelectorAll(
          ".homepage-testimonial__card-left, .homepage-testimonial__card-right",
        ),
      );
    }
    ...
    function getParts(card) {
      const img =
        card.querySelector("figure.elementor-image-box-img") ||
        card.querySelector(".elementor-image-box-img") ||
        card.querySelector(".elementor-image-box-wrapper");
    ...
    function animateTo(nextIndex, direction = 1) {
      ...
      getCards(curPage).forEach((card, idx) => {
        ...
        tl.to(texts, { opacity: 0, duration: 0.4, ease: "power1.in" }, 0);
      });

      // Entry animations for next page - starts after all exits complete
      const enterStart = 0.5;
      getCards(nxtPage).forEach((card, idx) => {
        const { img, texts } = getParts(card);
        const imgAt = enterStart + idx * CARD_DELAY;
        ...
        tl.to(texts, { opacity: 1, duration: 0.5, ease: "power2.out" }, 0.1);
      });
    }
